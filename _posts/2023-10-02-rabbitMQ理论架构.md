...
有了交换机后，我们就可能根据交换机和队列、以及路由key之间的映射关系实现很多复杂的能力。   
比如：   
（1）实现消息广播的能力，将一条消息同时推送到多个Queue中，从而被多个消费者同时消费。   
（2）实现定向推送消息的能力，将一条消息只推送到某一个Queue中，从而只能被订阅该Queue的消费者消费，其他消费者无法消费。   
（3）实现模糊推送消息的能力，将一条消息提送到某一批Queue中，只要订阅这一批Queue中某个Queue的消费者都可以消费到这条消息。   
......
...
其他的消息都将被丢失。   
```youtrack
1.一个交换机，可以通过一个Routing key绑定一个Queue，这时候Exchange 和 Queue就是一对一的关系，实现定向转发。   
2.一个交换机，可以通过指定多个Routing key绑定同一个Queue，这时候Exchange和Queue也是一对一的关系，但实现了消息的模糊匹配，比如上面案例的black和green路由键。   

```

Exchange也可以使用同一个Routing key绑定到不同的Queue中去，这种场景下，一个交换机绑定了多个Queue，实现了消息广播的行为，这时候和Fanout交换机的行为是一致的。   
...
（2）一个生产者将消息投递到多个队列：
直连交换机需要指定同一个Routing key与多个Queue建立绑定关系，即可实现（广播行为，类似Fanout扇形交换机）。   
（3）多个生产者将消息投递到同一个队列：   
...
```
如上，routing key1可以同时映射两个队列queue1和queue2，这种实现了广播行为；   
...
（2）一个生产者将消息投递到多个队列：
Topic exchange使用同一个精确的Routing key或者包含模糊符号的Routing key，或者是任意匹配（只有#）与多个Queue建立绑定关系，即可实现（广播行为，类似Fanout扇形交换机）。   
（3）多个生产者将消息投递到同一个队列：  
...
扇形交换机也叫做广播交换机，它是最基本的交换机类型。它能做的事情非常简单，就是广播消息。   
