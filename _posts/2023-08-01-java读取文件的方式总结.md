...
（5）class和classLoader都提供getResourceAsStream()的方式直接返回目标资源流。这个特性特别重要，当我们把java工程打成jar包后，工程里面的资源文件就只能通过这种直接返回Stream的方式来处理，如果直接尝试读取一个file是找不到文件的，因为jar包里面的file说白了没有绝对的磁盘路径，因为我们直接通过class或者classLoader直接读取目标文件的Stream.这才是读取jar包中资源文件的正确姿势。            
