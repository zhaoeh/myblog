...
# 6. BeanPostProcessor 整体生命周期逻辑梳理
这个BeanPostProcessor实际上，关于它的回调执行时机，总是反反复复被搞乱。有时候总会产生很多迷惑，比如：    
（1）假如一个bean，自己实现了 BeanPostProcessor，那么关于它自己的实例化过程是怎样的呢？它自己的实例化会不会是一个完整的调度链（即完整的走完spring中关于一个单例bean的整体实例化、初始化流程，会不会被AOP代理？等等）。      
（2）假如这个bean，自己实现了 BeanPostProcessor，那么这个bean内部通过@Autowire注入的其他bean，其交给spring容器管理的过程是完整的吗？会不会注入的是一些早期的bean实例或者是null？   
（3）同理，一个bean自己实现了 BeanPostProcessor，又实现 ApplicationContextAware，那么 setApplicationContext()方法和 postProcessAfterInitialization()方法的调度顺序是怎样的？   
（4）假如一个bean自己实现了 BeanPostProcessor ，然后在 postProcessAfterInitialization()方法中，又通过 applicationContext 对象手动getBean获取容器中的其他bean实例，那么这个手动获取的bean的生命周期是完整的吗？会走完容器中的所有处理器吗？    
等等等等。   
这里面的核心要分为几个部分去看：   
（1）容器内的早期内部bean自身的实例化过程，凡是实现了 BeanPostProcessor 接口的bean，或者实现了spring容器内部其他钩子接口的bean，这些bean都会在spring容器启动的早期阶段进行实例化。关于早期bean的实例化，很显然，这些早期bean 自身的实例化是没法走完spring容器中的所有处理器链路的，因此，早期bean中依赖的其他bean，很可能不是完整的bean，因为早期bean本身在实例化的过程中，其他bean还没有实例化完毕呢。   
所以，从早期bean自身实例化的过程来分析，就会得出一些结论：    
如果一个bean自己实现了 BeanPostProcessor ，同时这个bean自身通过 @Autowired 注入了容器中的其他bean，此时会发生什么？   
此时，这些早期bean自身依赖的其他bean，可能是一个半成品，即属性可能没有成功填充，甚至 @Autowired 注解可能都没法识别，因为它本身也是依赖内部的 BeanPostProcessor去识别的。     
得出结论：    
早期bean自身的实例化过程，是完整的，但是它本身依赖的其他bean，由于过早依赖，那么这些依赖bean的实例化过程大多数都不是完整的，甚至可能是null，也可能不是null，但是AOP会失效等等。   
...
（3）普通bean的实例化过程，请注意，每一个普通bean的实例化前后都会全量执行容器中所有早期bean的回调方法，此时所有早期bean早已经优先实例化完毕了，也就是说第（2）步实际上就是在普通bean的实例化前后才进行触发的，而且此时所有的处理器链路都是完整的。这意味着，任何一个普通bean都会触发所有早期bean的回调方法，在早期bean的回调方法中可以手动获取任何一个bean，这个bean的生命周期都是完整的。   
