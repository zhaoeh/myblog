...
这意味着，任何一个普通bean的实例化前后都会触发所有早期bean的回调方法，在早期bean的回调方法中可以手动获取任何一个bean，这个bean的生命周期都是完整的。     
得出结论：    
在所有早期bean的 postProcessAfterInitialization() 等回调方法中，手动获取其他普通bean，这个阶段获取的bean对象会完整执行完spring容器中管理一个bean的所有生命周期。   
