...
等等等等。   
这里面的核心要分为几个部分去看：   
（1）容器内的早期内部bean，凡是实现了 BeanPostProcessor 接口的bean，或者实现了spring容器内部其他钩子接口的bean，这些bean都会在spring容器启动的早期阶段进行实例化。关于早期bean的实例化，很显然，这些早期bean 自身的实例化是没法走完spring容器中的所有处理器链路的，因此，早期bean中依赖的其他bean，很可能不是完整的bean，因为早期bean本身在实例化的过程中，其他bean还没有实例化完毕呢。   
（2）
