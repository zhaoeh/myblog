...
---
# 7.redisson内部如何保证获取锁的lock对象和释放锁的lock对象是同一个对象呢？
比如下面的伪代码逻辑👇
...
# 8. redisson的lock和tryLock重载方法详细解释 
非常好 👍——你提的这个问题是 **Redisson 分布式锁的核心机制**，涉及锁获取、续期、超时、看门狗等底层原理。
我会一步步系统讲清楚（含所有重载、参数语义、执行流程、自动续期机制、Lua 实现逻辑、常见坑、最佳实践），读完你就能完全理解 Redisson 锁的运作原理。

---

# 🧩 一、RLock 核心实现概览

Redisson 的 `RLock` 是基于 Redis 实现的 **可重入分布式锁（Reentrant Lock）**。
它的实现基于以下关键点：

1. **Redis 存储结构**：

   * 锁在 Redis 中以一个 Key 存在（默认：`"redisson_lock:{name}"`）。
   * 值为一个 **Hash**，结构如下：
     ...
     value:
       ...
       value: reentrantCount (整数)
     TTL: 30s（默认，看门狗会续期）
     ```
2. **锁持有者**由 `clientId + threadId` 唯一标识（Redisson 在每个 JVM 启动时生成 clientId）。
3. **支持可重入**（同一线程可多次加锁，计数递增）。
4. **通过 Lua 脚本原子执行**，确保多步命令不被其他线程打断。
5. **看门狗机制（lock watchdog）** 自动续期，避免锁在任务未完成时过期。

---

# ⚙️ 二、`lock()` 与 `tryLock()` 的所有重载方法对比

| 方法签名                                                            | 阻塞性              | 是否超时返回 | 是否自动续期              | 说明                                                         |
| --------------------------------------------------------------- | ---------------- | ------ | ------------------- | ---------------------------------------------------------- |
| `void lock()`                                                   | 阻塞（一直等）          | 否      | ✅ 是                 | 一直等待，直到获取锁；获取后自动启动看门狗。                                     |
| `void lock(long leaseTime, TimeUnit unit)`                      | 阻塞（一直等）          | 否      | ❌ 否                 | 一直等待，直到获取锁；锁持有 `leaseTime` 后自动释放，不启动看门狗。                   |
| `boolean tryLock()`                                             | 非阻塞              | ✅ 是    | ✅ 是                 | 尝试立即加锁，不成功立刻返回 false。成功则开启看门狗。                             |
| `boolean tryLock(long waitTime, TimeUnit unit)`                 | 阻塞（最多等 waitTime） | ✅ 是    | ✅ 是                 | 等待最多 waitTime 获取锁。成功则开启看门狗。                                |
| `boolean tryLock(long waitTime, long leaseTime, TimeUnit unit)` | 阻塞（最多等 waitTime） | ✅ 是    | ⚙️ 看 leaseTime 是否设置 | 等待 waitTime 内尝试加锁，成功后锁的过期时间为 leaseTime；leaseTime>0 时不启看门狗。 |
...
| 参数名         | 含义            | 默认行为                      | 注意事项                              |
| ----------- | ------------- | ------------------------- | --------------------------------- |
| `waitTime`  | 获取锁时最长等待时间    | 默认无限（lock()）或0（tryLock()） | 超过 waitTime 未获取锁则返回 false         |
| `leaseTime` | 锁自动释放时间（秒/毫秒） | 默认 -1（启动看门狗）              | 若手动指定 leaseTime > 0，则关闭看门狗        |
...
| 情况                                   | 是否启用看门狗                | 是否自动释放            | 备注               |
| ------------------------------------ | ---------------------- | ----------------- | ---------------- |
| `lock()`（无 leaseTime）                | ✅ 启用                   | ❌ 不会自动释放（除非程序宕机）  | 依靠看门狗续期维持        |
| `lock(leaseTime, unit)`              | ❌ 禁用                   | ✅ leaseTime 到期后释放 | 适合定时任务、明确执行时间的逻辑 |
| `tryLock(waitTime, leaseTime, unit)` | ⚙️ leaseTime>0 禁用，否则启用 | leaseTime 到期后释放   | 常用于有重试上限的分布式任务   |
| `tryLock()`                          | ✅ 启用                   | ❌ 不自动释放           | 立即尝试加锁           |

---

# ⏰ 四、看门狗机制（Lock Watchdog）

## 🔹 启动时机

* 当你调用 `lock()`、`tryLock()` 等未指定 `leaseTime` 的方法时，Redisson 默认启用“看门狗”机制。

## 🔹 机制说明

1. 获取锁后，Redisson 会在本地开启一个后台 **续期定时任务**（`LockWatchdogTimeout`）。
2. 默认超时时间：`internalLockLeaseTime = 30 秒`（配置项：`redisson.lockWatchdogTimeout`）。
3. 定时任务会每隔 **10 秒（internalLockLeaseTime / 3）** 去刷新锁 TTL（重置为 30 秒）。
4. 若客户端线程正常执行（未释放锁），TTL 会一直续期，锁不会过期。
5. 一旦持锁的客户端崩溃或线程退出，续期任务停止，TTL 不再刷新，锁在 30 秒后自动过期释放。
...
* 不需要手动指定锁持有时间，防止任务超长导致锁过期被误释放。
* 程序崩溃时锁能在超时后自动释放，避免死锁。

## 🔹 缺点

* 如果业务逻辑很短（几百毫秒），不指定 leaseTime 反而会多出 30 秒 TTL；
* 在极端高 QPS 场景可能会增加 Redis 续期请求（不过性能影响不大）。

---

# 🔄 五、锁的获取与释放过程（内部 Lua 逻辑）
...
if (not exists(KEY)) then
  -- key 不存在，新建锁
  hset(KEY, clientId:threadId, 1)
  pexpire(KEY, leaseTime)
  return nil
end
if (hexists(KEY, clientId:threadId)) then
  -- 可重入，加计数
  hincrby(KEY, clientId:threadId, 1)
  pexpire(KEY, leaseTime)
  return nil
end
return pttl(KEY)
...
```lua
-- 解锁 Lua 脚本逻辑
if (not hexists(KEY, clientId:threadId)) then
  return nil  -- 当前线程不是持有者
end
local counter = hincrby(KEY, clientId:threadId, -1)
if (counter > 0) then
  pexpire(KEY, leaseTime) -- 可重入次数未归零，继续保留锁
  return 0
else
  del(KEY)
  publish(unlock_channel, "unlock")
  return 1
end
```
...
### ✅ 场景2：定时任务 / 明确任务时长（指定 leaseTime）

```java
RLock lock = redisson.getLock("jobLock");
if (lock.tryLock(0, 10, TimeUnit.SECONDS)) { // 不等待，锁10秒
    try {
        // 预计执行时间 < 10s
    } finally {
        if (lock.isHeldByCurrentThread()) {
...
| 坑点                   | 原因       | 后果                               | 解决方案                            |
| -------------------- | -------- | -------------------------------- | ------------------------------- |
| 手动指定 leaseTime 但任务超时 | 看门狗关闭    | 锁过期、被他人抢占、数据不一致                  | 估计足够长时间或使用自动续期                  |
| 未指定 leaseTime 但业务短   | 看门狗续期30s | 锁延迟释放，影响性能                       | 任务短则手动指定短 leaseTime             |
| unlock() 前线程崩溃       | 看门狗失效    | 锁在 30s 后释放                       | 正常机制，不是 bug                     |
| unlock() 未检查持锁者      | 多线程环境误解锁 | 抛 `IllegalMonitorStateException` | 使用 `isHeldByCurrentThread()` 校验 |
| 多 JVM 使用同名锁，时钟不一致    | 无影响      | Redisson 使用 TTL，不依赖本地时钟          | —                               |
...
| ------------------------------------------ | ------------------ | ----------- |
| `RLock`                                    | 可重入单锁              | 普通分布式锁      |
| `RFairLock`                                | 公平锁，排队机制           | 公平获取锁（FIFO） |
| `RReadWriteLock`                           | 读写锁                | 高并发读低并发写场景  |
| `RMultiLock`                               | 联锁（多个锁同时成功才算）      | 跨资源一致性锁     |
| `RRedLock`                                 | Redlock 算法（多节点多数派） | 高可靠集群锁      |
| `RSemaphore` / `RPermitExpirableSemaphore` | 信号量 / 限流           | 控制并发数       |
...
1. 常规场景用 `lock()` 或 `tryLock(waitTime, TimeUnit.SECONDS)`，让看门狗自动续期。
2. 明确执行时间短、确定的任务，用 `tryLock(0, leaseTime, unit)`。
3. 在 `finally` 中判断 `isHeldByCurrentThread()` 再 `unlock()`。
4. **永远不要用 `isLocked()` 判断是否可以加锁**（竞态）。
5. 高可靠要求 → 用 `RRedLock`（多个 Redis 节点）。
6. 调优参数：

   ```yaml
   redisson:
     lockWatchdogTimeout: 45000  # 根据业务耗时调整续期间隔
   ```
...
线程调用 lock()
   ↓
Redis 执行 Lua 脚本 -> 设置锁 Key（TTL=30s）
   ↓
Redisson 启动 Watchdog 定时任务（每10s刷新TTL）
   ↓
业务逻辑执行中
   ↓
unlock():
   → 校验是否当前线程
   → hincrby 计数归零后 DEL Key
   → 取消 Watchdog
```
